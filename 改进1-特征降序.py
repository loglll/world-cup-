...
df = df.loc[df.result != 0]  # 此处为求算法精度后，将平局去掉
a = df.columns.tolist()
a.sort()
data_set = df.loc[:,a]
# print(data_set.info())
...
X = data_set.drop(['result','home_score','away_score'],axis=1)
y = data_set['result']
...
b = df_group_match_2018.columns.tolist()
b.sort()
pred_set = df_group_match_2018.loc[:,b]
print(pred_set.info())
